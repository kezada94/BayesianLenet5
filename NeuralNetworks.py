...
class Lenet5Deterministic(nn.Module):
    def __init__(self):
        ...
        ## One output interpreted as the probability of being class 1
...
class Lenet5Bayesian(PyroModule):
    def __init__(self, prior_scale=2., isFEBayesian=True, prior_scaleFE = 2.):
        ...
        ## Make the feature extraction part of the NN bayesian?
        if isFEBayesian:
        ...
        ## Same as before, one value as output interpreted as a probability
        self.output = nn.Sigmoid()
        self.activation = nn.Tanh()
    ...
    def forward(self, x, y=None):
        ...
        with pyro.plate("data", f.shape[0]):
            ...
            obs = pyro.sample("obs", dists.Bernoulli(probs=loc), obs=y) #Bernoulli likelihood
